...
- [x] Extend the current prepared target/shader/surface-import caches into a clearer runtime GPU
      object cache model for textures, buffers, and bind groups
- [x] Key GPU object reuse explicitly by output/material/surface content version so present-time
...
- [x] Revisit crate/module boundaries now that the migration is complete, especially around the
      dispersed `wayland` runtime slices in `protocol` / `backend` / `input`
- [x] Split oversized plugin/orchestration modules into clearer runtime slices such as bootstrap,
