...
use nekoland_ecs::resources::{
    BackendInputAction, BackendInputEvent, ClipboardSelectionState, CompositorClock,
    KeyboardFocusState, PendingProtocolInputEvents, SelectionOwner,
};
...
fn clipboard_selection_roundtrips_between_two_real_clients() {
    ...
    assert!(source.selection_sent, "source client should set wl_data_device selection");
...
-> Option<(SourceClientSummary, TargetClientSummary, ClipboardSelectionState)> {
    ...
    app.inner_mut()
        .insert_resource(ClipboardTransferPump {
            source_selection_sent: source_selection_sent.clone(),
        })
        .add_systems(
            LayoutSchedule,
            pump_clipboard_transfer_input.after(decorations::server_decoration_system),
        );
    ...
    let socket_path = {
        ...
        match (&server_state.socket_name, &server_state.startup_error) {
            ...
            (None, Some(error)) if error.contains("Operation not permitted") => {
                eprintln!("skipping clipboard transfer test in restricted environment: {error}");
                return None;
            }
        ...
        while !target_flag.load(Ordering::SeqCst) {
            if Instant::now() >= deadline {
                return Err(common::TestControl::Fail(
                    "timed out waiting for source client to publish clipboard selection".to_owned(),
                ));
            }
...
-> Option<(SourceClientSummary, TargetClientSummary, ClipboardSelectionState)> {
    ...
    let socket_path = {
        ...
        match (&server_state.socket_name, &server_state.startup_error) {
            (Some(socket_name), _) => runtime_dir.path.join(socket_name),
            (None, Some(error)) if error.contains("Operation not permitted") => {
                eprintln!("skipping clipboard persistence test in restricted environment: {error}");
                return None;
            }
            (None, Some(error)) => panic!("protocol startup failed before run: {error}"),
            (None, None) => panic!("protocol startup produced neither socket nor error"),
    ...
    let target_thread = thread::spawn(move || {
        ...
        while !source_exited.load(Ordering::SeqCst) {
            if Instant::now() >= deadline {
                return Err(common::TestControl::Fail(
                    "timed out waiting for source client to exit after publishing clipboard selection"
                        .to_owned(),
                ));
            }
            thread::sleep(Duration::from_millis(10));
    ...
    while state.send_requests == 0 {
        ...
        if Instant::now() >= deadline {
            return Err(common::TestControl::Fail(format!(
                "timed out waiting for source send request (selection_sent={})",
...
fn run_target_client(socket_path: &Path) -> Result<TargetClientSummary, common::TestControl> {
    ...
    while state.received_payload != TEST_SELECTION_BYTES {
        ...
        if Instant::now() >= deadline {
            return Err(common::TestControl::Fail(format!(
                "timed out waiting for clipboard payload (receive_requested={}, offer_present={})",
...
impl Dispatch<wl_data_source::WlDataSource, ()> for TargetClientState {
    fn event(
        _state: &mut Self,
        _source: &wl_data_source::WlDataSource,
        _event: wl_data_source::Event,
        _data: &(),
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
    ) {
...
impl Dispatch<wl_data_offer::WlDataOffer, ()> for TargetClientState {
    fn event(
        ...
        if matches!(event, wl_data_offer::Event::Offer { mime_type } if mime_type == TEST_MIME_TYPE)
